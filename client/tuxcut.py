...
def dark_palette():
    ...
    p.setColor(QPalette.ColorRole.Base,             QColor(24, 24, 37))
    ...
    p.setColor(QPalette.ColorRole.Highlight,        QColor(137, 180, 250))
    p.setColor(QPalette.ColorRole.HighlightedText,  QColor(30, 30, 46))
    p.setColor(QPalette.ColorRole.ToolTipBase,      QColor(49, 50, 68))
    p.setColor(QPalette.ColorRole.ToolTipText,      QColor(205, 214, 244))
    return p
...
<g stroke="white" stroke-linecap="round" stroke-linejoin="round">
  <circle cx="11" cy="12" r="3" fill="none" stroke="white" stroke-width="1.8"/>
  <circle cx="21" cy="12" r="3" fill="none" stroke="white" stroke-width="1.8"/>
  <line x1="13.5" y1="14" x2="20" y2="22" stroke-width="2.2"/>
  <line x1="18.5" y1="14" x2="12" y2="22" stroke-width="2.2"/>
</g>
</svg>'''
...
ICO_MOON = b'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
<circle cx="16" cy="16" r="14" fill="#2c3e50"/>
<text x="16" y="22" text-anchor="middle" font-size="16"
  fill="white" font-family="sans-serif">&#9790;</text></svg>'''

ICO_SUN = b'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._gw           = {}
        self._my           = {}
        self._original_mac = ''   # الـ MAC الأصلي قبل أي تغيير
        self.live_hosts    = []
        self._offline_ips  = []
        ...
        if not self._server_ok():
            QMessageBox.critical(
                ...
                'فشل تشغيل الخادم!\n\n'
                'جرب:\n'
                '  sudo systemctl restart tuxcutd\n\n'
                'أو شغّل البرنامج من جديد.')
            sys.exit(1)

        self._load_gw()
        self._load_my(self._gw.get('iface', ''))
        self._original_mac = self._my.get('mac', '')  # حفظ الـ MAC الأصلي
        self._update_title()
        self.trigger_thread()

        # تحديث الـ IP كل 5 ثوانٍ
        self._ip_timer = QTimer(self)
        self._ip_timer.timeout.connect(self._refresh_my_info)
    ...
    def _build_ui(self):
        ...
        # Dark/Light toggle
        self.act_theme = QAction(mk_icon(ICO_MOON), 'Toggle Dark/Light Mode', self)
    ...
    def _apply_theme(self):
        dark = self.prefs.get('dark_mode', True)
        app = QApplication.instance()
        if dark:
            app.setPalette(dark_palette())
            self.act_theme.setIcon(mk_icon(ICO_SUN))
            self.act_theme.setToolTip('Switch to Light Mode')
        else:
            app.setPalette(app.style().standardPalette())
            self.act_theme.setIcon(mk_icon(ICO_MOON))
            self.act_theme.setToolTip('Switch to Dark Mode')

    def toggle_theme(self):
        self.prefs['dark_mode'] = not self.prefs.get('dark_mode', True)
    ...
    def _update_title(self):
        current_mac = self._my.get('mac', '')
        # إذا تغيّر الـ MAC، أظهر الأصلي والمؤقت
        if self._original_mac and current_mac != self._original_mac:
            mac_str = f'MAC: {self._original_mac} → {current_mac} (temp)'
        else:
            mac_str = f'MAC: {current_mac}'
        self.setWindowTitle(
            f'TuxCut-NG  —  '
            f'IP: {self._my.get("ip", "")}  |  '
            f'{mac_str}  |  '
            f'GW: {self._gw.get("ip", "")} [{self._gw.get("iface", "")}]')

    def _refresh_my_info(self):
        """تحديث IP و MAC في الوقت الفعلي."""
        iface = self._gw.get('iface', '')
        if not iface:
            return
        try:
            r = requests.get(f'{SERVER}/my/{iface}', timeout=3)
            if r.ok and r.json()['status'] == 'success':
                new_info = r.json()['my']
                if new_info != self._my:
                    self._my = new_info
                    self._update_title()
        except Exception:
            pass
    ...
    def trigger_thread(self):
        ...
        if not gw_ip:
            return
        # مرر IP البوابة والواجهة للسيرفر للحصول على مسح دقيق
        url = (f'{SERVER}/scan/{gw_ip}/{iface}' if iface
               else f'{SERVER}/scan/{gw_ip}')
        t = Worker(lambda: requests.get(url, timeout=90).json()['result']['hosts'])
    ...
    def on_resume(self, _=None):
        ...
        def _done(_):
            ...
            for col in range(1, 5):
                item = self.hosts_view.item(row, col)
                if item:
                    item.setForeground(QApplication.palette().color(QPalette.ColorRole.Text))
    ...
    def on_change_mac(self, _=None):
        ...
        try:
            ...
            if result.get('status') == 'success':
                ...
                # الـ MAC الأصلي لا يتغير — فقط المؤقت يتغير
                self._update_title()
                self.statusBar().showMessage(
                    f'MAC changed: {self._original_mac} → {new_mac}')
            else:
...
def ensure_root():
    """
    إذا لم يكن المستخدم root، يطلب صلاحية root مع الحفاظ على
    متغيرات Wayland/X11 حتى تعمل الواجهة الرسومية.
    """
    if os.geteuid() == 0:
        return  # already root
    ...
    # متغيرات البيئة الضرورية لـ Wayland و X11
    env_vars = {}
    ...
    # محاولة 1: sudo مع الحفاظ على متغيرات البيئة
    if shutil.which('sudo'):
        env_list = [f'{k}={v}' for k, v in env_vars.items()]
        cmd = ['sudo'] + env_list + [sys.executable, script] + sys.argv[1:]
        os.execvp('sudo', cmd)
        return

    # محاولة 2: pkexec (قد لا يعمل دائماً مع Wayland)
    if shutil.which('pkexec'):
        os.execvp('pkexec', ['pkexec', sys.executable, script] + sys.argv[1:])
        return

    print("ERROR: TuxCut-NG requires root. Run: sudo python3 client/tuxcut.py",
          file=sys.stderr)
    sys.exit(1)
...
if __name__ == '__main__':
    ...
    # تشغيل الخادم في الخلفية إذا لم يكن يعمل
    import subprocess, shutil, time
    try:
        r = requests.get('http://127.0.0.1:8013/status', timeout=2)
        ...
        if os.path.exists(server):
            subprocess.Popen(
                [sys.executable, os.path.abspath(server)],
