...
cat > /usr/local/bin/tuxcut << 'LAUNCHER'
#!/bin/bash
exec sudo WAYLAND_DISPLAY="$WAYLAND_DISPLAY" \
          XDG_RUNTIME_DIR="$XDG_RUNTIME_DIR" \
          DISPLAY="$DISPLAY" \
          DBUS_SESSION_BUS_ADDRESS="$DBUS_SESSION_BUS_ADDRESS" \
          python3 /opt/tuxcut-ng/client/tuxcut.py "$@"
LAUNCHER
chmod +x /usr/local/bin/tuxcut

cat > /usr/share/applications/tuxcut-ng.desktop << 'DESKTOP'
[Desktop Entry]
Version=1.0
Name=TuxCut-NG
